...
void create(int n)
{
    ...
    if (fptr != NULL)
    {
        ...
        for (j = 0; j < n; j++)
        {
            fprintf(fptr, "%02d %d %20s %20s %5s %4s %15s %d %d\n", j + 1, stu[j].idd, stu[j].fnn, stu[j].lnn, stu[j].gn, stu[j].brn, stu[j].city, st u[j].age, stu[j].jmarks);
        }
        memset(stu[j].gn, 0, 5);
...
void display()
{
    ...
    if (fptr != NULL)
    {
        ...
        for (j = 0; j < n; j++)
        {
            fscanf(fptr, "%d %d %s %s %s %s %s %d %d", &stu[j].recno, &stu[j].idd, stu[j].fnn, stu[j].lnn, stu[j].gn, stu[j].brn, stu[j].city, &stu[j].age, &stu[j].jmarks);
            printf("%02d %d %15s %15s %5s %4s %8s %d %d\n", stu[j].recno, stu[j].idd, stu[j].fnn, stu[j].lnn, stu[j].gn, stu[j].brn, stu[j].city, stu[j].age, stu[j].jmarks);
            memset(stu[j].gn, 0, 5);
            memset(stu[j].brn, 0, 4);
            memset(stu[j].fnn, 0, 20);
            memset(stu[j].lnn, 0, 20);
...
void create_indexfname()
{
    ...
    if (fptr != NULL)
    {
        ...
        fclose(fptr);
        fclose(fptr1);
printf("File Index_Fname.txt has been created and data has been
stored.\n");
    }
    else
...
void search()
{
    ...
    if (fptr != NULL)
    {
        ...
        if (found == 1)
        {
            ...
            fseek(fptr1, (i - 1) * (sizeof(struct student) + 4), SEEK_CUR); fscanf(fptr1,"%d %d %s %s %s %s %s %d
%d",&stu[0].recno,&stu[0].idd,stu[0].fnn,stu[0].lnn,stu[0].gn,stu[0].brn,stu[0 ].city,&stu[0].age,&stu[0].jmarks);
printf("%02d %d %s %s %s %s %s %d %d\n",stu[0].recno,stu[0].idd,stu[0].fnn,stu[0].lnn,stu[0].gn,stu[0].brn,stu[0 ].city,stu[0].age,stu[0].jmarks);
   fclose(fptr);
   fclose(fptr1);
        }
        else
...
void create_indexcity()
{
    ...
    if (fptr != NULL)
    {
        for (j = 0; j < n; j++)
        {
fscanf(fptr,"%d %d %s %s %s %s %s %d
%d",&stu[j].recno,&stu[j].idd,stu[j].fnn,stu[j].lnn,stu[j].gn,stu[j].brn,stu[j ].city,&stu[j].age,&stu[j].jmarks);
        }
        ...
        for (j = 0; j < n; j++)
        {
            ...
            fprintf(fptr1, "%02d %d %s %s %s %s %s %d %d\n", j + 1, stu[j].idd, stu[j].fnn, stu[j].lnn, stu[j].gn, stu[j].brn, stu[j].city, st u[j].age, stu[j].jmarks);
            printf("%02d %d %s %s %s %s %s %d %d\n", j + 1, stu[j].idd, stu[j].fnn, stu[j].lnn, stu[j].gn, stu[j].brn, stu[j].city, st u[j].age, stu[j].jmarks);
        }
        fclose(fptr);
        fclose(fptr1);
printf("File Index_City.txt has been created and data has been
stored.\n");
    }
    else
        printf("File not found.\n");
...
int main()
{
    ...
    do
    {
        ...
        printf("7.Execution time to display records city wise.\n"); printf("8.Display all the student records ascending order of city and
age.\n");
